function [position,positionaux,messages] = wavedet_3D_ECGKit(sig_all, ext_anot, heasig, messages )
...
if ~isfield(messages.setup.wavedet,'filter_bank_design') || messages.setup.wavedet.filter_bank_design~=1 % Rute 24/11/11
    ...
    [q1,q2,q3,q4,q5,messages] = qspfilt5_2(messages.setup.wavedet.freq,messages);  % WT equivalent filters % Rute 22/05/02
    if messages.status==1
    ...
    if( ~messages.setup.wavedet.QRS_detection_only )
        ...
        if ~isempty(time) %2JUN08 RUTE
            ...
            if ~isempty(intervalo1)
                [position1,qrspiconset1,qrspicoffset1,messages]= qrswavef_2(heasig,samp,time1,position1,w1,intervalo1,messages);
